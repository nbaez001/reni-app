...
import { RegPerfilComponent } from './reg-perfil/reg-perfil.component';

@Component({
  selector: 'app-bdj-perfil',
  templateUrl: './bdj-perfil.component.html',
  styleUrls: ['./bdj-perfil.component.scss']
})
export class BdjPerfilComponent implements OnInit {
  exportar = false;
  ...
  dataSource: MatTableDataSource<PerfilListarResponse>;
  ...
  columnsGrilla = [
    {
      columnDef: 'nombre',
      header: 'Nombre',
      cell: (m: PerfilListarResponse) => (m.nombre != null) ? `${m.nombre}` : ''
    }, {
      columnDef: 'flgActivo',
      header: 'Estado',
      cell: (m: PerfilListarResponse) => this.activoLista.filter(el => (el.id == m.flgActivo))[0].nombre
    }];
  ...
  buscar(): void {
    ...
    this.perfilService.listarPerfil(req).subscribe(
      (data: OutResponse<PerfilListarResponse[]>) => {
        if (data.rcodigo == CONSTANTES.R_COD_EXITO) {
          this.listaPerfil = data.objeto;
        } else {
          this.listaPerfil = [];
        }
  ...
  exportarExcel() {
    ...
    this.perfilService.exportarListaPerfilXlsx(req).subscribe(
  ...
  regPerfil() {
    const dialogRef = this.dialog.open(RegPerfilComponent, {
      width: '600px',
      disableClose: false,
      data: {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.listaPerfil.unshift(result);
        this.cargarDatosTabla();
      }
    });
  }

  editPerfil(obj: PerfilListarResponse) {
    let index = this.listaPerfil.indexOf(obj);
    const dialogRef = this.dialog.open(ModPerfilComponent, {
      width: '600px',
      disableClose: false,
      data: {
        titulo: MENSAJES_PANEL.INTRANET.ACCESOS.PERFIL.MODIFICAR.TITLE,
        objeto: obj
      }
    });
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.listaPerfil.splice(index, 1, result);
        this.cargarDatosTabla();
      }
    });
  }

  elimPerfil(obj: PerfilListarResponse): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result == CONSTANTES.COD_CONFIRMADO) {
        ...
        let req = new PerfilEliminarRequest();
        req.idPerfil = obj.idPerfil;
        req.idUsuarioModif = this.user.getIdUsuario;

        this.perfilService.eliminarPerfil(req).subscribe(
